...
Ext.define('NewsPaper.controller.RestoralController', {
	...
	views : ['RestoralBaseContainer', 'RestoralGridContainer',
			'RestoralGridView'],
	stores : ['RestoralStore', 'FilterEmployeeStore'],
	...
	restoralGridRender : function(grid) {
		...
		store.on('beforeload', function() {
					var queryCondition = grid
							.down('#filterRestoralEmployeeCondition')
					...
					if (getCookie("ghtn_user") == "") {
						...
						Ext.Ajax.request({
							...
							success : function(response) {
								...
								if (result.success) {
									Ext.example.msg('恭喜', '复职成功！');
								} else {
									Ext.example.msg('对不起', '复职失败！');
								}
								store.reload();
							},
